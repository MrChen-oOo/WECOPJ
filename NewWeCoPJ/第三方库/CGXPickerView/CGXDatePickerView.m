                      ...
                      Manager:(CGXPickerViewManager *)manager
{
    if (self = [super init]) {
        ...
        // 默认选中今天的日期
        if (defaultSelValue.length > 0 && !kStringIsEmpty(defaultSelValue)) {
...
- (UIDatePicker *)datePicker {
    if (!_datePicker) {
        ...
        [_datePicker setValue:COLOR(102,102,102,1) forKey:@"textColor"];
        
        _datePicker.frame = CGRectMake(0, self.manager.kTopViewH + 0.5, SCREEN_WIDTH, self.manager.kPickerViewH);
        _datePicker.backgroundColor = [UIColor whiteColor];//COLOR(1, 31, 40, 1);
...
//        [_datePicker setDate:[NSDate date] animated:YES];
        ...
        NSDate *reDate = [formater2 dateFromString:_selectValue];
        [_datePicker setDate:reDate animated:YES];
