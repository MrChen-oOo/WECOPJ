...
- (void)initUI{
    ...
    [self.rightButton setTitle:root_YHQ_551 forState:UIControlStateNormal];
...
- (void)setSelectTime:(NSString *)selectTime{
    ...
    if (self.selectTime.length > 0) {
        ...
        [self.rightButton setTitle:kStringIsEmpty(array[1]) ? root_YHQ_551 : array[1] forState:UIControlStateNormal];
        
        leftButtonString = kStringIsEmpty(array[0]) ? root_kaishishijian : array[0];
        rightButtonString = kStringIsEmpty(array[1]) ? root_YHQ_551 : array[1];
    }
}
