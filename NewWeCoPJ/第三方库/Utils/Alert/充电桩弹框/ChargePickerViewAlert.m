...
#import "ChargePickerViewAlert.h"// 充电桩时间选择弹框

@interface ChargePickerViewAlert ()<UIPickerViewDelegate,UIPickerViewDataSource>{
...
@implementation ChargePickerViewAlert
...
- (void)initUIView{
    ...
    UIButton *cancelBtn = [[UIButton alloc]initWithFrame:CGRectMake(10*XLscaleW, 15*XLscaleH, 80*XLscaleW, 35*XLscaleH)];
...
//    if ([_whereInString isEqualToString:@"solarLinkI"]) {
//        UIButton *notSetBtn = [[UIButton alloc]initWithFrame:CGRectMake(ScreenWidth/6, 15*XLscaleH, ScreenWidth*2/3, 35*XLscaleH)];
//        notSetBtn.titleLabel.font = FontSize(18*XLscaleW);
//        [notSetBtn setTitle:root_weishezhi forState:UIControlStateNormal];
//        [notSetBtn setTitleColor:COLOR(0, 156, 255, 1) forState:UIControlStateNormal];
//        [notSetBtn addTarget:self action:@selector(notSetBtnClick) forControlEvents:UIControlEventTouchUpInside];
//        [self.viewBG addSubview:notSetBtn];
//    }
    
    UIButton *enterBtn = [[UIButton alloc]initWithFrame:CGRectMake(ScreenWidth - 90*XLscaleW, 15*XLscaleH, 80*XLscaleW, 35*XLscaleH)];
    ...
    float textW = 60*XLscaleW, textH = 15*XLscaleH, textX = ScreenWidth/2 - 30*XLscaleW, textY = 40*XLscaleH+self.pickerView.xmg_height/2-textH/2;
    UILabel *hourLB = [[UILabel alloc]initWithFrame:CGRectMake(textX, textY, textW, textH)];
    hourLB.text = root_shi;
    hourLB.font = FontSize([NSString getFontWithText:hourLB.text size:hourLB.xmg_size currentFont:14*XLscaleH]);
    ...
    minLB.text = root_fen;
    minLB.font = FontSize([NSString getFontWithText:minLB.text size:minLB.xmg_size currentFont:14*XLscaleH]);
