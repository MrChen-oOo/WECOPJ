...
#import "InputSettingAlert.h"// 文本输入弹框
...
- (void)initUI{
    ...
    inputField.attributedPlaceholder = attri;
           [inputField setMinimumFontSize:textFiedMinFont];
...
- (void)setItemText:(NSString *)itemText{
    ...
    if ([_itemText isEqualToString:@"rate"] || [_itemText isEqualToString:@"power"] || [_itemText isEqualToString:@"number"]) {
...
- (void)setPlaceholderText:(NSString *)PlaceholderText{
    ...
    NSMutableParagraphStyle *style = [[NSMutableParagraphStyle alloc] init];
    style.alignment = NSTextAlignmentCenter;
    if (!kStringIsEmpty(PlaceholderText)) {
        NSAttributedString *attri = [[NSAttributedString alloc] initWithString:PlaceholderText attributes:@{NSFontAttributeName:FontSize(13*XLscaleW), NSParagraphStyleAttributeName:style}];
         self.inputField.attributedPlaceholder = attri;
             [self.inputField setMinimumFontSize:textFiedMinFont];
    }
